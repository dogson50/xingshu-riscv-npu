...
set report_dir [file join $root_dir reports \
    v13_systolic_cluster_runtime_stream_300m]
...
read_verilog [file join $root_dir rtl v13 compute \
    npu_v13_systolic_cluster_runtime_stream.v]
read_xdc [file join $root_dir constraints npu_v13_300m.xdc]

synth_design -mode out_of_context \
    -top npu_v13_systolic_cluster_runtime_stream \
    -part xc7a200tfbg484-2 -flatten_hierarchy rebuilt \
    -directive PerformanceOptimized

# OOC 边界没有上层 SRAM/FIFO 的物理位置，因此仍排除端口路径。
# 与静态版不同，本模块在端口后增加了源寄存器；A/B/shape/valid
# 从该寄存器 Q 经运行时广播选择器到每核边界寄存器 D 的路径属于
# internal reg-to-reg，会被下面的 300 MHz 门禁真实检查。
...
write_checkpoint -force [file join $report_dir runtime_cluster_synth.dcp]
...
write_checkpoint -force [file join $report_dir runtime_cluster_routed.dcp]
...
puts $summary "top=npu_v13_systolic_cluster_runtime_stream"
puts $summary "part=xc7a200tfbg484-2"
puts $summary "configuration=runtime_hierarchical_broadcast"
...
puts $summary "task_abort_supported=0"
puts $summary "runtime_clear_port=removed"
puts $summary "source_port_bits_a_plus_b=1024"
puts $summary "active_source_bits_16x16=256"
puts $summary "active_source_bits_4x8x8=512"
puts $summary "active_source_bits_16x4x4=1024"
puts $summary "shape_port_bits_rows_plus_cols=64"
puts $summary "shape_packing=mode_compact_group_index"
...
puts $summary "runtime_router_path=internal_reg2reg_included"
puts $summary "internal_reg2reg_wns_ns=$internal_wns"
puts $summary "internal_reg2reg_whs_ns=$internal_whs"
close $summary

if {$internal_wns < 0.0} {
    error "300 MHz runtime cluster setup failed: WNS=$internal_wns ns"
}
if {$internal_whs < 0.0} {
    error "300 MHz runtime cluster hold failed: WHS=$internal_whs ns"
}

puts "NPU_MYDESIGN_V13_RUNTIME_CLUSTER_300M_IMPL_PASS dsp48e1=$dsp_count lut_total=[expr {$logic_lut_count + $srl_count}] ff=$ff_count internal_wns_ns=$internal_wns internal_whs_ns=$internal_whs"
