...
// 输入状态：FREE -> LOADING -> READY -> IN_USE -> FREE；装载失败回 FREE。
// 输出状态：FREE -> IN_USE -> READY -> DRAINING -> FREE；执行失败直接回 FREE。
...
// 单次消费策略：正常或错误 release 都释放 A/B；复用权重需要以后显式扩展保留策略。
// reset 必须和 executor/装载器/feeder/collector 同步中止任务。只清状态，不清数据/tag。
module npu_v13_buffer_manager #(
    ...
    always @(posedge clk or negedge resetn) begin
        if(!resetn) begin
            ...
            if(release_fire_w) begin
                a_state_r[owner_a_r]<=FREE; b_state_r[owner_b_r]<=FREE;
                c_state_r[owner_c_r]<=buffer_release_error_i?C_FREE:C_READY;
                lease_r<=0;
            end
