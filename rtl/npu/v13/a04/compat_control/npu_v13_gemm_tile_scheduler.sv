...
// 隔离实验r4：仅将首批终点判断改成直接比较；正式原件保留不动。
// command_sources.tcl显式选择本副本，禁止与正式同名模块同时加入工程。
//
// 一、模块职责
// -----------
...
module npu_v13_gemm_tile_scheduler #(
    ...
    // 对合法正尺寸：floor((dimension-1)/step)==0 等价于 dimension<=step。
    // 原表达式串联“全宽减一 -> 步长选择/右移 -> 零比较 -> M/N合并”；
    // 终点标志直接比较原尺寸，可以绕开前面的减法进位链。剩余batch计数
    // 仍使用上面的cmd_*_after_w，运行时M/N/K、所有mode/layout与输出拍数不变。
    // dimension=0的命令仍由command_valid_f拒绝，不能依赖本标志放行非法命令。
    wire cmd_m_last_w = (cmd_m_i <= cmd_step_m_w);
    wire cmd_n_last_w = (cmd_n_i <= cmd_step_n_w);
