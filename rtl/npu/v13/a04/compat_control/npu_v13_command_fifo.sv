...
// 隔离实验r3：复制正式FIFO，仅修正BRAM控制复位契约；源SHA256见PLAN.md。
// 不与正式同名模块同时编译，command_sources.tcl显式选择本副本。
// resetn必须已同步到clk且低电平覆盖至少一个上升沿；复位期间接口禁止握手。
//
// 一、模块职责
// -----------
...
module npu_v13_command_fifo #(
    ...
    input  wire                             resetn, // 低有效同步复位计数和有效状态，逻辑清空队列；不清零 RAM/数据寄存器内容。
    ...
    wire ram_read_en_w =
        resetn && (unread_count_r != {COUNT_W{1'b0}}) && ram_slot_open_w;
    ...
    // 指针/计数/valid同步复位，避免异步复位寄存器直接驱动BRAM地址/EN/REGCE。
    // RAM和两个宽数据级仍不复位；低有效valid会屏蔽复位前的陈旧数据。
    always @(posedge clk) begin
        if (!resetn) begin
