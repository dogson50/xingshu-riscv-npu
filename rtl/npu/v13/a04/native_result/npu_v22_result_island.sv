...
module npu_v22_result_island #(
    ...
    input wire reserve_valid_i, // 已获准的不可撤销 COMMIT；本拍无条件记账，ready 只用于前一拍探测。
    ...
    output wire [3:0] native_reserve_o,
    input wire [3:0] capture_ready_i,
    input wire [3:0] row_valid_i,
    output wire [3:0] row_ready_o,
    input wire [511:0] row_data_i,
    input wire [15:0] row_keep_i,
    input wire [7:0] row_index_i,
    input wire [191:0] row_meta_i,
    input wire [3:0] row_error_i,
    ...
    wire [3:0] cv,cr;
    assign cv=row_valid_i;
    assign row_ready_o=cr;
    wire [4*PW-1:0] cp;
    wire [2:0] request_count={2'b0,reserve_mask_i[0]}+{2'b0,reserve_mask_i[1]}+
                             {2'b0,reserve_mask_i[2]}+{2'b0,reserve_mask_i[3]};
    assign reserve_ready_o=resetn && (&(capture_ready_i | ~reserve_mask_i)) &&
        ({1'b0,used_r}+request_count<=TOTAL_SLOTS);
    // P85-A：issue 只有在上一拍四岛 ready 全部成立后才进入 COMMIT。
    // sole-producer/stable-mask 契约保证 grant 不会撤销；COMMIT 不再重新经过
    // request_count/capture_ready/capacity 组合锥后才驱动 16 个 collector 的 LUTRAM WE。
    wire reserve=resetn && reserve_valid_i;
    // P87-G: local one-cycle COMMIT token. The global issue-state decode now ends at
    // four small island-local registers instead of directly driving sixteen
    // collector metadata LUTRAM write enables across the tile array. Packet SEND
    // timing is unchanged; descriptor metadata remains stable through the delayed
    // native reservation cycle by the issue-controller state contract.
    reg [3:0] native_reserve_r;
    always @(posedge clk) begin
        if(!resetn) native_reserve_r<=4'b0;
        else native_reserve_r<=reserve ? reserve_mask_i : 4'b0;
    end
    assign used_o=used_r;
    assign empty_o=used_r==0;
    assign retire_o=out_valid_o && out_ready_i && out_tile_last_o;
    assign error_o=accounting_error_r || |row_error_i;
    assign native_reserve_o=native_reserve_r;
    genvar t;
    generate for(t=0;t<4;t=t+1) begin:G_NATIVE_ROW
        wire [1:0] row=row_index_i[t*2+:2];
        wire [47:0] meta=row_meta_i[t*48+:48];
        wire [15:0] m=meta[31:16]+TILE_ROW*4+row;
        wire [15:0] n=meta[15:0]+t*4;
        assign cp[t*PW+:PW]={row==3,meta[47:32],m,n,row_keep_i[t*4+:4],row_data_i[t*128+:128]};
    end endgenerate
    wire mv,mr;
    wire [PW-1:0] md,od;
    ...
    always @(posedge clk) begin
        if(!resetn) begin used_r<=0;accounting_error_r<=0;end
            ...
            if((retire_o && used_r==0) || used_r>TOTAL_SLOTS || (reserve_valid_i && !reserve_ready_o)) accounting_error_r<=1;
        end
    end
    // synthesis translate_off
    initial if(TOTAL_SLOTS<4 || CAPTURE_SLOTS<2 || TILE_ROW>3) $fatal(1,"island parameters");
    always @(posedge clk) if(resetn && reserve_valid_i && !reserve_ready_o)
        $fatal(1,"island irrevocable commit without prior stable grant");
