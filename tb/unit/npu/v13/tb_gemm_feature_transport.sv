...
`timescale 1ns/1ps
// 正式端口联合：装载真实A/B BRAM -> 命令/256DSP/跨K -> 量化 -> feature读回。
// 黄金模型按完整K独立计算，并独立实现整数舍入；不以DUT输出/内部RAM作为参考。
module tb_gemm_feature_transport;
    parameter integer STALL=1,RESET_ABORT=0,TRANSPORT_W=64;
    localparam MN=32,MK=16,AW=5,TAW=AW+(TRANSPORT_W==64),EW=TRANSPORT_W+TRANSPORT_W/8+51;
    ...
    reg disv=0,disop=0,disbank=0;reg [TAW-1:0] la=0;reg [TRANSPORT_W-1:0] ld=0;
    ...
    reg rv=0,rb=0,rr=0,rlast=0;wire olast;reg [15:0] rm=0,rn=0;wire rready,ov,ob,oe;wire [TRANSPORT_W-1:0] od;wire [TRANSPORT_W/8-1:0] ok;wire [15:0] om,on,ot;
    npu_v13_gemm_feature_transport #(.TRANSPORT_W(TRANSPORT_W),.MAX_MN(MN),.MAX_K(MK),.TOTAL_SLOTS(16),.FIFO_DEPTH(4)) dut(
        ...
        .load_valid_i(lv),.load_ready_o(lr),.load_operand_i(lo),.load_bank_i(lb),.load_addr_i(la),.load_data_i(ld),.load_keep_i({TRANSPORT_W/8{1'b1}}),
        ...
        .rd_last_i(rlast),.rd_last_o(olast),.rd_valid_i(rv),.rd_ready_o(rready),.rd_bank_i(rb),.rd_m_i(rm),.rd_n_i(rn),.rd_valid_o(ov),.rd_ready_i(rr),
    ...
    reg [EW-1:0] expected[0:8191];integer wp=0,rp=0,last_request=-10,last_commit=-10;
    integer gathered=0,loadbeats=0,nativewrites=0;
    reg [TRANSPORT_W-1:0] qdata;reg [TRANSPORT_W/8-1:0] qkeep;reg qb,qerr;reg [15:0] qt,qm,qn;
    reg rh=0,dh=0,hold_read=0,read_fullrate=0;reg [EW-1:0] rheld;reg [51:0] dheld;
    always @(negedge clk)if(resetn)begin allow=!STALL || cycles%11!=3;rr=!hold_read && (read_fullrate || !STALL || cycles%13<8);end
    always @(posedge clk)begin
        if(!resetn)begin rh=0;dh=0;gathered=0;end
        else begin
            cycles=cycles+1;if(error)$fatal(1,"GEMM_TRANSPORT backend protocol error");
            if(beat)beats=beats+1;if(packet)packets=packets+1;
            if(lv && lr)loadbeats=loadbeats+1;
            // p10：计数真正的bank本地128位RAM提交，不把窄口握手冒称128位写。
            if(|dut.u_core.u_compute.u_backend.u_memory.u_ram.local_pending)nativewrites=nativewrites+1;
            ...
            if(rh && (!ov || {olast,ob,oe,ot,om,on,ok,od}!==rheld))$fatal(1,"read response changed while stalled");
            rh=ov && !rr;rheld={olast,ob,oe,ot,om,on,ok,od};
            if(rv && rready)begin: QUEUE
                ...
                if(gathered==0)begin qdata=0;qkeep=0;qb=rb;qerr=0;qt=tag;qm=rm;qn=rn;end
                for(integer z=0;z<4;z=z+1)begin qdata[(gathered*4+z)*8+:8]=data[z*8+:8];qkeep[gathered*4+z]=mask[z];end
                qerr=qerr || bad;gathered=gathered+1;
                if(rlast || gathered==TRANSPORT_W/32)begin expected[wp]={rlast,qb,qerr,qt,qm,qn,qkeep,qdata};wp=wp+1;gathered=0;end
                requests=requests+1;
                if(|fc || |inuse)overlap=overlap+1;if(cycles==last_request+1)ii1=ii1+1;last_request=cycles;
            end
            if(ov && rr)begin
                if(rp==wp || {olast,ob,oe,ot,om,on,ok,od}!==expected[rp])$fatal(1,"GEMM_TRANSPORT read mismatch index=%0d actual=%h expected=%h",rp,{olast,ob,oe,ot,om,on,ok,od},expected[rp]);
    ...
    task automatic load_panel(input integer operand,bank,epoch,kbase);
        begin if(retained[operand*2+bank])discard(operand,bank);
            ...
            for(integer block=0;block<MN/16;block=block+1)for(integer k=0;k<MK;k=k+1)begin
                for(integer h=0;h<128/TRANSPORT_W;h=h+1)begin
                    lv=1;la=(block*MK+k)*(128/TRANSPORT_W)+h;
                    for(integer l=0;l<TRANSPORT_W/8;l=l+1)ld[l*8+:8]=sample(operand,epoch,block*16+h*(TRANSPORT_W/8)+l,kbase+k);
                    do @(posedge clk);while(!lr);@(negedge clk);
                end
            end
    ...
    task automatic finish_job(input integer status);
        begin wait(dv);@(negedge clk);
            if(ds!==status[2:0] || dt!==jtag || db!==jbank)$fatal(1,"GEMM_TRANSPORT done mismatch status=%0d expected=%0d",ds,status);
    ...
    task automatic matrix(input integer bank,m,n,k0,k1,k2,epoch,axis,tag);
        ...
        begin total=k0+k1+k2;nc=k2>0 ? 3 : (k1>0 ? 2 : 1);
            ...
            if(beats-basebeats!=((m+15)/16)*((n+15)/16)*total || packets-basepackets!=((m+15)/16)*((n+15)/16)*nc || commits-basecommits!=m*((n+3)/4))
                $fatal(1,"GEMM_TRANSPORT throughput/event conservation");end
    endtask
    task automatic scan(input integer bank,repeats);
        begin for(integer r=0;r<repeats;r=r+1)for(integer m=gm[bank]-1;m>=0;m=m-1)for(integer n=0;n<gn[bank];n=n+4)begin
            @(negedge clk);rv=1;rb=bank;rm=m;rn=n;rlast=(n+4>=gn[bank]);do @(posedge clk);while(!rready);end
    ...
    initial begin
        ...
        matrix(0,29,21,16,16,5,1,0,16'h6100);
        fork
            matrix(1,17,29,9,9,1,2,1,16'h6101);
            scan(0,5);
        join
        ...
        @(negedge clk);jv=0;
        read_fullrate=1;scan(1,2);read_fullrate=0;release_feature(0);
        matrix(0,32,32,3,0,0,3,0,16'h6102);scan(0,2);release_feature(0);release_feature(1);
        ...
        // 不重装A/B，真实验证RETAINED输入供下一父任务复用；搬运计数不得增加。
        begin: REUSE_RETAINED
            integer before_load;before_load=loadbeats;
            start_job(1,1,1,1,5,1,16'h6104);chunk(1,0);finish_job(0);scan(1,1);release_feature(1);
            if(loadbeats!=before_load)$fatal(1,"GEMM_TRANSPORT retained reuse reloaded data");
        end
        if(successes!=5 || failures!=3 || wp!=rp || loadbeats!=nativewrites*(128/TRANSPORT_W) || overlap==0 || load_overlap==0 || holds==0 || blocked!=31 || ii1<100)
            $fatal(1,"GEMM_TRANSPORT coverage missing");
        $display("GEMM_TRANSPORT_PASS successes=%0d failures=%0d beats=%0d packets=%0d commits=%0d reads=%0d overlap=%0d load_overlap=%0d held_done=%0d capacity_blocked=%0d ii1_reads=%0d reset_abort=%0d width=%0d load_beats=%0d native_writes=%0d output_beats=%0d cycles=%0d",successes,failures,beats,packets,commits,requests,overlap,load_overlap,holds,blocked,ii1,RESET_ABORT,TRANSPORT_W,loadbeats,nativewrites,responses,cycles);
        $finish;
    end
    initial begin #2000000;$fatal(1,"GEMM_TRANSPORT TIMEOUT");end
endmodule
