...
module gemm_feature_timing #(
    ...
    assign {resetn,jv,jm,jn,jt,jk,jfb,jaxis,xv,xk,xab,xbb,xcb,xa,xb,dr,
        ...
        lbv,lbo,lbb,lv,lo,lb,la,ld,lfv,lfo,lfb,lfe,disv,disop,disbank,allow,
        frelv,frelb,rv,rb,rm,rn,orr}=launch_r;
    always @(posedge clk) begin
        launch_r<=stimulus_i;
        observation_o<={jr,xr,dv,dt,ds,dfb,kd,cfgr,cfge,lbr,lbe,lr,lfr,lfer,disr,dise,
            retained,loading,inuse,fready,ffree,frelr,rr,ov,od,ok,ob,oe,om,on,ot,
            pcommit,fcommit,beat,packet,busy,error};
    end
    npu_v13_gemm_feature_backend #(.MAX_MN(MAX_MN),.MAX_K(MAX_K)) u_dut(
        ...
        .rd_valid_i(rv),.rd_ready_o(rr),.rd_bank_i(rb),.rd_m_i(rm),.rd_n_i(rn),.rd_valid_o(ov),.rd_ready_i(orr),
    ...
    initial begin
        if($bits({resetn,jv,jm,jn,jt,jk,jfb,jaxis,xv,xk,xab,xbb,xcb,xa,xb,dr,
            cfgv,ci,cbias,cmult,cshift,czp,crelu,
            lbv,lbo,lbb,lv,lo,lb,la,ld,lfv,lfo,lfb,lfe,disv,disop,disbank,allow,
            frelv,frelb,rv,rb,rm,rn,orr})!=IW) $fatal(1,"GEMM_FEATURE fixture input width");
        if($bits({jr,xr,dv,dt,ds,dfb,kd,cfgr,cfge,lbr,lbe,lr,lfr,lfer,disr,dise,
            retained,loading,inuse,fready,ffree,frelr,rr,ov,od,ok,ob,oe,om,on,ot,
            pcommit,fcommit,beat,packet,busy,error})!=OW) $fatal(1,"GEMM_FEATURE fixture output width");
        $display("GEMM_FEATURE_FIXTURE_WIDTH_PASS IW=%0d OW=%0d",IW,OW);
    end
