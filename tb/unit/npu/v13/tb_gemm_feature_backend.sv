...
module tb_gemm_feature_backend;
    parameter integer STALL=1,RESET_ABORT=0;
    localparam MN=32,MK=16,AW=5;
    ...
    reg [85:0] expected[0:8191];integer wp=0,rp=0,last_request=-10,last_commit=-10;
    ...
    always @(posedge clk)begin
        if(!resetn)begin rh=0;dh=0;end
            ...
            if(rv && rready)begin: QUEUE
                ...
                expected[wp]={rb,bad,tag,rm,rn,mask,data};wp=wp+1;requests=requests+1;
                if(|fc || |inuse)overlap=overlap+1;if(cycles==last_request+1)ii1=ii1+1;last_request=cycles;
            end
            if(ov && rr)begin
                if(rp==wp || {ob,oe,ot,om,on,ok,od}!==expected[rp])$fatal(1,"GEMM_FEATURE read mismatch index=%0d actual=%h expected=%h",rp,{ob,oe,ot,om,on,ok,od},expected[rp]);
    ...
    initial begin
        ...
        $display("GEMM_FEATURE_PASS successes=%0d failures=%0d beats=%0d packets=%0d commits=%0d reads=%0d overlap=%0d load_overlap=%0d held_done=%0d capacity_blocked=%0d ii1_reads=%0d reset_abort=%0d",successes,failures,beats,packets,commits,requests,overlap,load_overlap,holds,blocked,ii1,RESET_ABORT);
        $finish;
    end
    initial begin #2000000;$fatal(1,"GEMM_FEATURE TIMEOUT");end
endmodule
