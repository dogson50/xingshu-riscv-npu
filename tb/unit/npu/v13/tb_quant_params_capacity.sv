...
`timescale 1ns/1ps
// 默认128通道；黄金表按真实cfg握手更新，不读取DUT层次/内部RAM。
module tb_quant_params_capacity;
...
`else
    npu_v13_quant_params #(.CHANNELS(128)) dut(
`endif
    ...
    reg [77:0] golden[0:127];reg [127:0] initialized=0;
    ...
    always @(posedge clk)begin
        if(!resetn)begin initialized=0;held=0;fired=0;end
            ...
            if(cv && cr)begin
                if(ce !== (ci>=128))$fatal(1,"PARAMS cfg error mismatch");
                if(ci<128)begin golden[ci]=cw;initialized[ci]=1;cfgwrites=cfgwrites+1;end
            ...
            for(g=0;g<4;g=g+1)begin
                if(iv[g] && ir[g])begin
                    ...
                    for(l=0;l<4;l=l+1)begin
                        ...
                        if(ik[g*4+l])begin
                            if(ix==127 && initialized[127])begin if(axis)high_n=high_n+1;else high_m=high_m+1;end
                            if(ix>=128 || (axis && incol[g*16+:2]!=0))bad=1;
            ...
            for(integer c=0;c<clocks;c=c+1)begin
                ...
                for(integer a=0;a<4;a=a+1)begin
                    ...
                    if(!iv[a] || fired[a])begin
                        ...
                        im[a*16+:16]=(v%133);incol[a*16+:16]=((v>>8)%34)*4;
    ...
    initial begin
        ...
        for(integer c=0;c<128;c=c+1)if(c!=7)configure(c,0);
        configure(128,0);configure(65535,0);
        ...
        for(integer c=0;c<128;c=c+1)configure(c,1);
        ...
        traffic(128,0);
        if(high_m==0 || high_n==0)$fatal(1,"PARAMS high channel untested");
        $display("PARAMS_CAPACITY_COVERAGE channel127_m=%0d channel127_n=%0d",high_m,high_n);
        if(accepted!=retired+canceled || canceled!=4 || errors<100 || cfgwrites!=255 || cfgerrors!=2 || fullrate<400 || stalls<100)
            $fatal(1,"PARAMS insufficient coverage");
        $display("PARAMS_CAPACITY_PASS accepted=%0d retired=%0d canceled=%0d errors=%0d writes=%0d cfgerrors=%0d stalls=%0d fullrate_cycles=%0d",accepted,retired,canceled,errors,cfgwrites,cfgerrors,stalls,fullrate);
        $finish;
    end
