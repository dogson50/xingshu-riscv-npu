...
module tb_requant_p4;
    ...
    reg [31:0] zp=0;reg [3:0] keep=0,relu=0;reg [31:0] meta=0;
    wire [31:0] od,om;wire [3:0] ok;
`ifdef POST_NETLIST
    npu_v13_requant_p4 dut(
`else
    npu_v13_requant_p4 #(.META_W(32)) dut(
`endif
    ...
    reg [31:0] expected_data[0:8191],expected_meta[0:8191];reg [3:0] expected_keep[0:8191];
    reg held=0,all_ready=0,force_stall=0;reg [67:0] payload;
    ...
    always @(negedge clk)if(resetn)orr=!force_stall && (all_ready || (cycles%17<11));
    ...
    initial begin
        ...
        // 连续512beat证明II=1；后续随机气泡/反压及一次200cycle长停顿。
        for(integer i=0;i<4608;i=i+1)begin
            @(negedge clk);all_ready=i<512;drive(i);iv=1;
            do @(posedge clk);while(!ir);
            if(i==700)begin @(negedge clk);iv=0;force_stall=1;repeat(200)@(negedge clk);force_stall=0;end
            if(i>=512 && i%5==0)begin @(negedge clk);iv=0;repeat(i%3)@(negedge clk);end
        end
        drain();
        // 停住最终输出后reset取消在途数据；随后新任务不能读到旧流水身份。
        @(negedge clk);force_stall=1;all_ready=0;
        for(integer i=0;i<3;i=i+1)begin @(negedge clk);drive(7000+i);iv=1;@(posedge clk);if(!ir)$fatal(1,"reset setup blocked");end
        @(negedge clk);iv=0;wait(ov);repeat(12)@(negedge clk);
        canceled=canceled+wr-rd;rd=wr;resetn=0;repeat(5)@(negedge clk);resetn=1;force_stall=0;all_ready=1;
        for(integer i=0;i<512;i=i+1)begin @(negedge clk);drive(8000+i);iv=1;do @(posedge clk);while(!ir);end
        drain();repeat(8)@(negedge clk);
        for(integer s=0;s<64;s=s+1)if(shift_seen[s]==0)$fatal(1,"missing shift %0d",s);
        if(accepted!=retired+canceled || canceled!=3 || stalls==0 || ii_count<900 || satlo==0 || sathi==0 || zero_mult==0 || ties==0 || empty==0)
            $fatal(1,"REQUANT missing coverage a=%0d r=%0d canceled=%0d ii=%0d",accepted,retired,canceled,ii_count);
        $display("REQUANT_PASS accepted=%0d retired=%0d canceled=%0d stalls=%0d ii1=%0d satlo=%0d sathi=%0d ties=%0d shifts=64",accepted,retired,canceled,stalls,ii_count,satlo,sathi,ties);$finish;
    end
