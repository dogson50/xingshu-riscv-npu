...
module tb_feature_store_capacity;
...
`else
    npu_v13_feature_store #(.MAX_M(128),.MAX_N(128)) dut(
`endif
    ...
    integer high_writes=0,high_reads=0;
    ...
    always @(posedge clk)if(!resetn)held=0;else begin
        ...
        for(integer g=0;g<4;g=g+1)begin
            if(iv[g] && ir[g])begin writes=writes+1;
                if(im[g*16+:16]==127 && incol[g*16+:16]==124)high_writes=high_writes+1;end
        ...
        if(rv && rready)begin: ENQUEUE
            ...
            if(wrp-rdp>=256)$fatal(1,"FEATURE scoreboard overflow");
            if(rm==127 && rn==124)high_reads=high_reads+1;
            bad=!live[rb] || rm>=gm[rb] || rn>=gn[rb] || rm>=128 || rn>=128 || rn%4!=0;
            ...
            expected[wrp%256]={rb,bad,tag,rm,rn,mask,data};wrp=wrp+1;requests=requests+1;
            if(|commit)overlap=overlap+1;
        end
        if(ov && rr)begin
            if(rdp==wrp || {ob,oe,ot,om,on,ok,od}!==expected[rdp%256])$fatal(1,"FEATURE read mismatch req=%0d got=%h expected=%h",rdp,{ob,oe,ot,om,on,ok,od},expected[rdp%256]);
    ...
    initial begin
        ...
        start_fill(0,127,125,101,1);write_panel(0,127,125,1,0);finish_fill(0,0);
        fork
            begin start_fill(1,117,127,202,2);write_panel(1,117,127,2,0);finish_fill(0,0);end
            read_panel(0);
        join
        ...
        read_panel(1);request(1,117,0);request(1,0,1);request(1,0,128);request(1,65535,0);wait(rdp==wrp);
        ...
        start_fill(1,128,128,408,8);write_panel(1,128,128,8,0);finish_fill(0,0);read_panel(1);release_bank(1);
        if(high_reads<2 || high_writes!=1)$fatal(1,"FEATURE last word coverage missing");
        $display("FEATURE_CAPACITY_COVERAGE row127_col124_wr=%0d rd=%0d",high_writes,high_reads);
        if(requests!=responses+canceled || canceled!=1 || overlap==0 || stalls==0 || successes!=4 || failures!=4 || commits!=writes-1)
        ...
        $display("FEATURE_CAPACITY_PASS writes=%0d commits=%0d requests=%0d responses=%0d canceled=%0d overlap=%0d stalls=%0d successes=%0d failures=%0d",writes,commits,requests,responses,canceled,overlap,stalls,successes,failures);$finish;
    end
